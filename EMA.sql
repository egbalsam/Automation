...
INTO	#MACD
FROM	#EMA
where	--strTick = 'aapl' AND
		--dtDate = (select max(dtDate) from #Stock_list) AND 
		(EMA23 IS NOT NULL or EMA50 IS NOT NULL)

IF OBJECT_ID('tempdb..#KFAST') IS NOT NULL DROP TABLE #KFAST 
SELECT	*,
		CASE
		WHEN MAX(MACD) OVER (ORDER BY strtick asc, dtDate ASC ROWS 9 PRECEDING) = MIN(MACD) OVER (ORDER BY strtick asc, dtDate ASC ROWS 9 PRECEDING)
		THEN NULL
		ELSE
		(100 * (MACD-MIN(MACD) OVER (ORDER BY strtick asc, dtDate ASC ROWS 9 PRECEDING))) / (MAX(MACD) OVER (ORDER BY strtick asc, dtDate ASC ROWS 9 PRECEDING)-MIN(MACD) OVER (ORDER BY strtick asc, dtDate ASC ROWS 9 PRECEDING))
		END as KFAST
INTO #KFAST
FROM #MACD
WHERE	dtDate >= dateadd(day,-365,(select max(dtDate) FROM #Stock_list))
ORDER BY strTick,ID

IF OBJECT_ID('tempdb..#DSLOW') IS NOT NULL DROP TABLE #DSLOW 
Select *,
		AVG(KFAST) OVER (ORDER BY strtick asc, dtDate ASC ROWS 2 PRECEDING) as DSLOW
INTO #DSLOW
FROM (SELECT * FROM #KFAST WHERE KFAST IS NOT NULL) kf

/*
IF OBJECT_ID('tempdb..#STC') IS NOT NULL DROP TABLE #STC 
Select *,
		100*((MACD - KFAST)/(DSLOW - KFAST)) as STC
INTO #STC
FROM #DSLOW
*/
/*Schaff = 100 x (MACD – %K (MACD)) / (%D (MACD) – %K (MACD))*/

select * from #KFAST where strTick = 'CSCO'