...
#tick = 'XLM'
ticklist = [ 'BTC','ETH','XLM','ADA','DOGE']

tick = input('Enter ticker: ')

if tick not in ticklist:
...
if timeint == '1m':
    data = yf.download(tickers= tick + '-USD',period = '1d', interval = timeint)
    sma = 630
    lma = 2700
elif timeint == '2m':
    data = yf.download(tickers= tick + '-USD',period = '2d', interval = timeint)
    sma = 315
    lma = 1350
elif timeint == '5m':
    data = yf.download(tickers= tick + '-USD',period = '5d', interval = timeint)
    sma = 123
    lma = 540
elif timeint == '15m':
    data = yf.download(tickers= tick + '-USD',period = '15d', interval = timeint)
    sma = 41
    lma = 180
elif timeint == '30m':
    data = yf.download(tickers= tick + '-USD',period = '30d', interval = timeint)
    sma = 21
    lma = 90
elif timeint == '90m':
    data = yf.download(tickers= tick + '-USD',period = '60d', interval = timeint)
    sma = 7
    lma = 30
elif timeint == '1h':
    data = yf.download(tickers= tick + '-USD',period = '60d', interval = timeint)
    ...
    data = yf.download(tickers= tick + '-USD',period = '365d', interval = timeint)
elif timeint == '5d':
    data = yf.download(tickers= tick + '-USD',period = '365d', interval = timeint)
elif timeint == '1w':
    data = yf.download(tickers= tick + '-USD',period = '3650d', interval = timeint)
elif timeint == '1mo':
    data = yf.download(tickers= tick + '-USD',period = '3650d', interval = timeint)
elif timeint == '3mo':
    data = yf.download(tickers= tick + '-USD',period = '3650d', interval = timeint)
