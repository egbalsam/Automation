...
#Ichimoku_01.00.py..............................Exchanged snp500 table with snp500.
...
try:
	...
	SELECT	ID,
			strTick,
			dtDate,
			CAST(decOpen as decimal(16,6)) AS decOpen,
			CAST(decHigh as decimal(16,6)) AS decHigh,
			CAST(decLow as decimal(16,6)) AS decLow,
			CAST(decAdjClose as decimal(16,6)) AS decClose,
			CAST(decAdjClose as decimal(16,6)) AS decAdjClose,
			CAST(LEAD(decOpen,1) OVER (PARTITION BY strTick ORDER BY dtDate ASC) as decimal(16,6)) as NextOpen,
			CAST(AVG(decAdjClose) OVER (PARTITION BY strTick ORDER BY dtDate ASC ROWS 20 PRECEDING) as decimal(16,6)) as SMA21,
			CAST(STDEV(decAdjClose) OVER (PARTITION BY strTick ORDER BY dtDate ASC ROWS 20 PRECEDING)*2 as decimal(16,6)) as BollBand21
	INTO BOLLINGER
	FROM snp500
	...
	JOIN snp500 snp ON snp.strTick = ibuy.strTick
	...
	script = """
			...
			AND dtdate  = (select MAX(dtdate) from snp500)
			"""
